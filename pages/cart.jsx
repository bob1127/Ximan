      ...
      <main className="min-h-screen bg-white text-[#121212] font-sans pt-24 md:pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          ...
          <div className="flex justify-between items-end mb-10 md:mb-16">
            ...
            <Link href="/category" className="text-sm underline underline-offset-4 hover:text-[#ef4628] transition-colors">
              繼續購物
            </Link>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-1 md:col-span-6 flex gap-6">
                        <Link href={`/product/${item.id}`} className="relative w-24 h-32 md:w-32 md:h-40 bg-gray-50 flex-shrink-0 overflow-hidden">
                          <Image
                        ...
                        <div className="flex flex-col justify-center gap-1">
                          ...
                          <Link href={`/product/${item.id}`} className="text-sm md:text-base font-bold text-black hover:underline decoration-1 underline-offset-2">
                            {item.title}
                          </Link>
                          ...
                          <div className="mt-4 md:hidden flex items-center justify-between w-full max-w-[200px]">
                             <div className="flex items-center border border-gray-300 h-8 w-24">
                                ...
                                <button onClick={() => handleQuantity(item, 'plus')} className="w-8 flex items-center justify-center hover:bg-gray-100">
                                  <Plus size={14} />
                                </button>
                             </div>
                             <button onClick={() => removeFromCart(item.id)} className="text-gray-400 hover:text-red-500 ml-4">
                                <Trash2 size={18} />
                             </button>
                          </div>
                        </div>
              ...
              <div className="flex flex-col items-end mt-10 md:mt-16 gap-4">
                <div className="w-full md:w-1/3 flex flex-col gap-4">
                  <div className="flex justify-between items-baseline border-b border-gray-100 pb-4">
                    <span className="text-base font-bold text-gray-600 uppercase tracking-widest">小計</span>
                    <div className="text-right">
                       <span className="text-2xl font-bold text-black">NT$ {subtotal.toLocaleString()}</span>
                       <p className="text-[10px] text-gray-400 mt-1">含稅。運費將於結帳步驟計算。</p>
                    </div>
                  </div>
